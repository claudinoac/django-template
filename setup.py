...
with open(os.path.join(current_directory, "django_template/requirements/production.txt"), 'r') as requirements:  # Caminho do arquivo com as dependências da aplicação em production mode
    REQUIREMENTS = requirements.read().splitlines()

setup(
    name='django template', # nome do pacote/aplicação
    version='0.0.1.dev4', # Versão do pacote -> Deve ser atualizada a cada deploy
    packages=['django_template'], # O nome da pasta com o conteúdo da aplicação.
    include_package_data=True,
    entry_points={'console_scripts': [
        'manage_application = django_template:manage',  # Criação do comando de administração da app (função manage dentro de django_template/__init__.py).
    ]},
